...
tinyowl 对用户提供的 OpenAI 兼容中转站接口进行多轮、六维度技术探测，输出 0–100 的可信度评分与维度明细，帮助用户识别「模型身份替换」「降智」「协议不一致」等风险；同时维护按模型分类的中转站榜单、官方 API 状态监控、检测历史与科普内容。
...
- **接口检测**：输入中转站地址、API Key、目标模型，发起多维度检测，实时进度（SSE）。
  - 六维度：返回协议一致性、响应结构、知识问答结果、身份一致性、思维链痕迹、签名指纹。
  - 可选缓存检测模式（约 +30s）。
